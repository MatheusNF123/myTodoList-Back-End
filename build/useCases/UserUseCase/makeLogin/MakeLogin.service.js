...
class MakeLoginService {
    constructor(userRepository) {
        this.login = ({ email, password }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user)
                throw new customError_1.default('Incorrect email or password', 401);
            const validPassword = yield bcrypt.compare(password, user.password);
            if (!validPassword)
                throw new customError_1.default('Incorrect email or password', 401);
            const token = GenerateToken_1.default.generateToken({ id: user.id, email });
            const { username } = user;
...
exports.default = MakeLoginService;
